...
local function CreateInstance(InstanceName: string, Information: {[string]: any}): any
...
local function CreateRemoteFunction(Name: string): Folder
	local Folder: Folder = CreateInstance("Folder", {
		Name = Name,
...
function LBConnection.RemoteFunction(Name: string, Info: {TimeOut: number?, RateLimit: number?, RateLimitTime: number?})
	...
	if (RemotesFolder:FindFirstChild(Name) ~= nil) then
		...
		if (IsServer) then
			...
			RemoteConnection:FireServer(false, Name);
		end
...
RS2.PostSimulation:Connect(function()
